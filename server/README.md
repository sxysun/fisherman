...
Fisherman Cloud, the same ingest code runs on the managed EC2 backend
with the production storage and account configuration.

In Cloud, `ingest.py` serves structured `/health`, receives client
uploads, and stores encrypted frame blobs through the configured storage
backend. A static `ENCRYPTION_KEY` is used for self-hosted/server-wrapped
mode or explicit one-off legacy migration.
...
| `ENCRYPTION_KEY` | Self-hosted/server-wrapped only | (auto-generated by setup.sh) | Master Fernet key for self-hosted rows and legacy migration |
| `FISH_PRIVATE_KEY` | Self-hosted only | (auto-generated by setup.sh) | Ed25519 private key (hex). In self-hosted mode this identifies the server owner. Cloud tenant mode accepts each user's FishKey directly |
...
| `FISH_CLOUD_KEY_MODE` | Cloud only | unset | Cloud key mode. EC2-hosted Cloud currently uses the server-wrapped model |
...
on local disk. Hosted Cloud should stay `closed` or `allowlist` until an
account/billing enrollment service provisions rows in `users`; `open` is
only appropriate for trusted private deployments.
...
In managed EC2 Cloud, rows use the server-wrapped key model unless a
future migration explicitly enables another key mode.
...
backend server, enroll and operate tenants with:
