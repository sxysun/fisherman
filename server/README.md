...
Postgres service, receives tenant data keys from approved clients, and
stores encrypted frame blobs on the CVM volume. External Postgres and R2
credentials remain supported overrides. A static `ENCRYPTION_KEY` is only
for self-hosted/server-wrapped mode or explicit one-off legacy migration.

## Quick Start

Recommended: let an agent handle the server side for you.

The current self-hosted auth shape is **public-key allowlist**, not
"copy the server private key to the Mac". Keep the user's Mac identity in
`~/.fisherman/.env`, allowlist that Mac's signing public key on the
server, and then point the Mac at the server URL.
...
Copy the `signing:` public key. On the server:
...
Auth model:
- FishKey Ed25519 auth is the only setup path documented for new
  deployments.
- Remote self-hosted servers should allowlist the Mac's signing public
  key. Do not copy private keys between machines.
...
4. Auto-generates `ENCRYPTION_KEY` (Fernet)
...
| `ENCRYPTION_KEY` | Self-hosted/server-wrapped only | (auto-generated by setup.sh) | Master Fernet key for self-hosted rows and legacy migration. Strict Cloud mode does not use this for new tenant data |
...
| `FISH_MULTI_TENANT` | No | unset | Enable tenant-scoped mode. Each allowed FishKey pubkey becomes its own user namespace |
| `FISH_ENROLLMENT_MODE` | No | `closed` | Self-hosted-friendly alias for enrollment gate: `open`, `allowlist`, or `closed` |
| `FISH_ALLOWED_PUBKEYS` | No | — | Self-hosted-friendly alias for comma/space-separated tenant pubkeys allowed to auto-enroll |
| `FISH_KEY_MODE` | No | `server_wrapped` | Self-hosted-friendly alias for tenant key mode: `server_wrapped` or `client_provided` |
...
| `FISH_CLOUD_KEY_MODE` | Cloud only | unset | Set `client_provided` for managed Cloud privacy mode. Approved clients provide tenant keys per runtime session |
| `FISH_CLOUD_LEGACY_DECRYPT_ENABLED` | Cloud migration only | unset | When true, permits loading a legacy Cloud wrapping key so old server-wrapped rows can be migrated |
...
Managed Cloud should also run with `FISH_CLOUD_KEY_MODE=client_provided`.
R2 credentials are optional; when absent, encrypted frame blobs are stored
on local CVM disk. Hosted Cloud should stay `closed` or `allowlist` until
an account/billing enrollment service provisions rows in `users`; `open`
is only appropriate for trusted private deployments.
...
server owner's key derived from `FISH_PRIVATE_KEY` when multi-tenant mode
is off. For a remote self-hosted server, prefer `FISH_MULTI_TENANT=1`
with `FISH_ENROLLMENT_MODE=allowlist` and `FISH_ALLOWED_PUBKEYS=<mac
signing pubkey>`, so the Mac keeps its existing identity and the server
only accepts explicitly allowed users.
...
In managed Cloud (`FISH_CLOUD_KEY_MODE=client_provided`), the approved
client sends the tenant data key to the attested runtime session and new
rows are marked `data_key_source=client_provided`. Older server-wrapped
rows are unavailable unless a migration runtime explicitly enables legacy
decrypt and has the old wrapping key.
...
Tenant enrollment commands require `DATABASE_URL`. Commands that decrypt
self-hosted/server-wrapped historical rows also need the matching
`ENCRYPTION_KEY`; client-held Cloud rows are only decryptable in a
runtime that has received the tenant key from an approved client.
`disable` and `revoke-device` stop future access without deleting
captured data.
